...
var settingsData = localStorage.settingsData ? JSON.parse(localStorage.settingsData) : [ {employer: '', company: '', emailAddr: '', emailCC: '', emailSubj: '' } ];
...
function validateTime(oldTime) {
	...
	if ( minutes < 11 ) newMinutes = 0;
	if (( minutes > 10 ) && ( minutes < 23 )) newMinutes = 15;
	if (( minutes > 22 ) && ( minutes < 42 )) newMinutes = 30;
	if (( minutes > 41 ) && ( minutes < 51 )) newMinutes = 45;
	if ( minutes > 50 )
		hours++;
...
function buildTimeDiff(begins, ends) {
	...
	if ( parseInt(begins.split(timeSeparator)[1],10) == 15 ) durationStr -= 0.25;
	if ( parseInt(begins.split(timeSeparator)[1],10) == 30 ) durationStr -= 0.5;
	if ( parseInt(begins.split(timeSeparator)[1],10) == 45 ) durationStr -= 0.75;
	
	if ( parseInt(ends.split(timeSeparator)[1],10) == 15 ) durationStr += 0.25;
	if ( parseInt(ends.split(timeSeparator)[1],10) == 30 ) durationStr += 0.5;
	if ( parseInt(ends.split(timeSeparator)[1],10) == 45 ) durationStr += 0.75;
...
function exportHeader(preview) {
	...
	exportHtml += '<tr><td style="padding: 2px 23px 2px 0;"' + nowrap + '>Company</td><td style="padding: 2px 23px 2px 0;"' + nowrap + '>' + settingsData[0].company + '</td></tr>';
	exportHtml += '<tr><td style="padding: 2px 23px 2px 0;' + nowrap + '">Employer name</td><td style="padding: 2px 23px 2px 0;"' + nowrap + '>' + settingsData[0].employer + '</td>';
	exportHtml += '</tr></table>';
...
function exportDB(preview) {
	...
	exportHtml += '<td style="width: auto; border: solid 1px #000; padding: 2px;"' + nowrap + '><b>Day</b></td>';
	exportHtml += '<td style="width: auto; border: solid 1px #000; border-left: 0; padding: 2px;"' + nowrap +'><b>Date</b></td>';
	exportHtml += '<td style="width: auto; border: solid 1px #000; border-left: 0; padding: 2px;" nowrap="nowrap"><b>Time</b></td>';
	exportHtml += '<td style="width: auto; border: solid 1px #000; border-left: 0; padding: 2px;"' + nowrap + '><b>Duration</b></td>';
	exportHtml += '<td style="width: auto; border: solid 1px #000; border-left: 0; padding: 2px;"' + nowrap + '><b>Event</b></td>';
	if ( needsInfo )
		exportHtml += '<td style="width: auto; min-width: 36px; border: solid 1px #000; border-left: 0; padding: 2px;"' + nowrap +'><b>Info</b></td>';
	exportHtml += '</tr>';
...
function exportSummary(preview) {
	...
	exportHtml += '<td style="padding-right: 10px;" nowrap="nowrap">Amount of hours:</td>';
	exportHtml += '<td style="padding-left: 5px; padding-right: 10px; border-bottom: solid 1px #000;" nowrap="nowrap">' + validateSummary(amountHours) + 'h</td>';
	exportHtml += '</tr>';
	exportHtml += '</table>';	
...
$$('.ac-report').on('click', function () {
	...
	if ( settingsData[0].emailCC.length > 0 ) cc += 'cc=' + settingsData[0].emailCC + '&';
		
    var buttons = [
        {
            text: 'Which parts to select for exporting?',
            label: true
        },
        {
            text: 'All',
    ...
    var buttons = [
        ...
        },
        {
            ...
            onClick: function () {
            				...
            				localStorage.hoursData = JSON.stringify(hoursData.sort(function(a, b){return a.row-b.row}));
            				buildHoursListHtml();
...
$$('.addhoursbtn').on('click', function () {
    ...
    if (( date.length == 0 ) || ( description.length == 0 )) {
       myApp.alert("Enter atleast date and description.");
       return;
    }    
    
    for (var i = 0; i < hoursData.length; i++)
        if (hoursData[i].row > row ) row = hoursData[i].row + 1;
...
function buildHoursListHtml() {
    ...
    for (var i = 0; i < hoursData.length; i++) {
        ...
        html += hourItemTemplate
                    .replace(/{{date}}/g, buildShortDate(hoursData[i].date))
                    .replace(/{{description}}/g, descStr)
                    .replace(/{{duration}}/g, durationStr)
                    .replace(/{{id}}/g, hoursData[i].id);
    }
...
window.addEventListener('load', function (e) {
    window.applicationCache.addEventListener('updateready', function (e) {
        if (window.applicationCache.status === window.applicationCache.UPDATEREADY) {
            ...
            myApp.confirm('A new version of Hours is available. Do you want to load it right now?', function () {
